...
import { Cars } from 'shared/types/types';
import './race.scss';

export const raceGenerator = ({ name, color, id }: Cars) => {
  const template = `
    <div class="race_line" data-id="${id}">
      <div class="wrapper line_wrap">
        <button class="blue_button select_btn">select</button>
        <button class="blue_button remove_btn">remove</button>
        <p class="name">${name}</p>
      </div>
      <div class="wrapper">
        <button class="controller start_btn">A</button>
        <button class="controller stop_btn disabled_controller">B</button>
        <svg version="1.0" fill="${color}" class="car_svg" xmlns="http://www.w3.org/2000/svg" width="50.000000pt" height="50.000000pt" viewBox="0 0 50.000000 50.000000" preserveAspectRatio="xMidYMid meet">
